...
const populateGenreDropdown = (genres) => {
  ...
  // Returns the current cast selection from the actor text input
  const getCastValue = () => {
    const castValue = document.getElementById('actors').value;
    return castValue;
  };
  
  // Returns the user cast choice by selecting actor id first in movie/person array
  ...
  // Returns rating color selection string by using rating average parameters
  ...
  // Create function object to load users liked movies
  const loadLikedMovies = () => JSON.parse(localStorage.getItem('likedMovies'));
  
  // Create function object to load users disliked movies
  ...
  // Clear all of users stored movie likes/dislikes from storage
