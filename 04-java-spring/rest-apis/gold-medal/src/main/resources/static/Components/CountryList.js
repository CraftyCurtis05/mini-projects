...
class CountryList extends React.Component {
  ...
  componentDidMount() {
    GoldMedalMetrics.getCountries('name', 'y').then(countries => {
      if(countries.length) {
  ...
  renderCountryRows() {
    ...
    return this.state.countries.map((country, index) => {
      return (
        <tr key={index}><td className="country-link"><Link to={`/countries/${country.name}`}>{country.name}</Link></td>
        <td>{country.gdp}</td>
    ...
    return (
      <table className="country-table">
        <tbody><tr className="table-header country-header">
          ...
          <th># OF GOLD MEDALS
            <img className="sort" alt="Sort Ascending" src="img/up.svg" onClick={() => this.sortBy('medals', true)}/>
            <img className="sort" alt="Sort Descending" src="img/down.svg" onClick={() => this.sortBy('medals', false)}/>
          </th>
        </tr>
